...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        Loop while l <= r.
            Calc m, cur.
            If cur == target, then return m.
            If cur >= nums[0], then m is in the left half of the array.
                If target >= nums[0] and target < cur, then target should be to the left, so r = m - 1. Otherwise,
                l = m + 1.
            If cur < nums[0], then m is in the right half of the array.
                If target <= nums[-1] and target > cur, then target should be to the right, so l = r + 1. Otherwise,
                r = m -1.
        Return -1
        Time: O(log(n))
        Space: O(1)
        Edge: None, but can only be one node.
        '''
        l, r = 0, len(nums)-1
        while l <= r:
            m = (l + r) // 2
            ...
            if cur == target:
                ...
                # m is in the left half
                if target >= nums[0] and target < cur:
                    r = m - 1
                else:
                    l = m + 1
            else:
                # m is in the right half
