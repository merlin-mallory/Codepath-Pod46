...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        Plan:
        Binary Search
        Determine if mid is in the left_seg or right_seg.
            if mid_v == target, return mid.
            Determine if target is in the left_seg or right_seg (and also which side of mid is the target)
                l = m + 1 or r = m - 1 as appropriate.
        return -1
        Time: O(log(n))
        Space: O(1)
        Edge: None
        ...
        while l <= r:
            ...
            if cur == target:
                ...
                if (target >= nums[0]) and (target < cur):
                    # target is in left_seg to the left
                    r = m - 1
                else:
                    # target is either in left_seg to the right of mid, or target is in right_seg
                    l = m +1
            else:
                # cur is in the right seg
                if (target <= nums[-1]) and (target > cur):
                    # target is in the right_seg to the right
                    l = m + 1
                else:
                    # target is either in right_seg to the left of mid, or target is in left_seg
                    r = m - 1
        return -1
