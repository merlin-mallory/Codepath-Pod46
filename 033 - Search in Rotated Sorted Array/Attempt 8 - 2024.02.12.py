...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        while l <= r:
            ...
            Compare cur and nums[0]. If cur >= nums[0], then we are in the left segment. Otherwise we're in the right
            segment. Then check if the target is in the same segment to the limit (search towards limit), or opposite.
        return -1
        '''
        l, r = 0, len(nums)-1
        while l <= r:
            ...
            if cur >= nums[0]:
                ...
                if (target > nums[-1]) or (target < cur):
                    r = m - 1
                else:
                    l = m + 1
        return -1
