...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        Loop while l <= r.
            Calc m, cur.
            Check if cur == target, if so, return m
            Check if cur >= nums[0] (m is in left seg)
                Check if target >= nums[0] and target < cur, search left with r = m - 1.
                Otherwise search right with l = m +1
            Otherwise, (m is in right seg)
                Do the mirror
        Return -1
        Time: O(log(n))
        Space: O(1)
        ...
        while l <= r:
            ...
            if cur == target: return m
                ...
                if target >= nums[0] and target < cur:
                    # target is in the left seg to the left of cur
                    r = m - 1
                else:
                    # target is in left seg to the right of cur, or is in the right seg
                    l = m + 1
            else:
                # m is in right seg
                if target <= nums[-1] and target > cur:
                    # target is in the right seg to the right of cur
                    l = m + 1
                else:
                    # target is in the right seg to the left of cur, or is in the left seg
                    r = m - 1
        return -1
