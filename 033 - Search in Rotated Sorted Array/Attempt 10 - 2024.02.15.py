...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        Loop while l <= r.
            ...
            Else if cur >= nums[0], then m is in the left subarray.
                If target < cur and target >= nums[0], then we need to search left, so r = m-1.
                Otherwise the target is either in the left subarray to the left, or the right subarry, so l = m+1
            Else, then m is in the right subarray.
                Mirror the if checks.
        return -1
        Time: O(log(n))
        Space: O(1)
        Edge: None
