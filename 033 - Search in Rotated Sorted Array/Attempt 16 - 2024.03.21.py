...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        Loop while l <= r.
            Calc m, grab cur.
            Check if the target is in the left_seg.
                Check if m is in the left_seg to the left of target. If so, r = m -1. Otherwise, l = m+1
            Otherwise, the target is in the right_seg.
                Check if m is in the right_seg to the right of target. If so, l = m + 1. Otherwise, r = m -1.
        Return -1.
        Time: O(log n)
        Space: O(1)
        Edge: None
