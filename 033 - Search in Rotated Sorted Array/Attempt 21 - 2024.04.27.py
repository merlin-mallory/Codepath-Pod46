...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        Time: O(log(n))
        Space: O(1)
        Edge: return -1 if target is not in nums
        '''
        l, r = 0, len(nums)-1
        while l <= r:
            m = (l + r) // 2
            ...
            if cur >= nums[0]:
                ...
                if (target >= nums[0]) and (target < cur):
                    ...
                    # target is either in right seg, or in left seg to the right of cur
                    l = m + 1
            else:
                ...
                if (target <= nums[-1]) and (target > cur):
                    ...
                    # target is either in left seg, or in right seg to the left of cur
                    r = m - 1
        return - 1
