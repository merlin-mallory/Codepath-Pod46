...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        '''
        # Binary Search
        # 1. Calc mid and cur, and handle cur == target.
        l, r = 0, len(nums)-1
        while l <= r:
            m = (l + r) // 2
            cur = nums[m]
            if cur == target:
                return m
            # 2. Determine if mid is in either the left sorted subarray or right sorted subarray.
            elif cur >= nums[0]:
                # Cur is in the left sorted subarray
                if (target >= nums[0]) and (target < cur):
                    r = m - 1
                else:
                    l = m + 1
            else:
                # Cur is in the right sorted subarray
