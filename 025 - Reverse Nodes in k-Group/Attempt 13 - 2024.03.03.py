...
class Solution:
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        Plan:
        Create fetch_kth_node(node) function. Returns the desired node if it exists, otherwise None.
        Create dummy linked to head.
        Set left_seg = dummy
        Loop while True.
            Call fetch_kth_node(left_seg), set equal to mid_seg.
            If not mid_seg, break.
            Set right_seg = mid_seg.next
            Reverse all of the nodes in mid_seg.
