...
class Solution:
    def reverseKGroup(self, head: Optional[ListNode], k: int) -> Optional[ListNode]:
        ...
        0 <= Node.val <= 1000
        """
        # Create fetch_kth_node(node). Returns the desired node.
        ...
        # Init left seg to dummy
        left_seg = dummy
        # Loop while True.
        while True:
            # Call fetch_kth_node on left_seg, set equal to mid_seg.
            mid_seg = fetch_kth_node(left_seg, k)
            # If not mid_seg, then break.
            if not mid_seg: break
            # Set right_seg = mid_seg.next
            right_seg = mid_seg.next
            # Reverse k nodes from left_seg.next to the end of mid_seg.
            prev = right_seg
            cur = left_seg.next
            while cur != right_seg:
                temp = cur.next
