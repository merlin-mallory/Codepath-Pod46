...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        Plan:
        Binary Search
        Calc total_len, half_len.
        Set the shorter len array to A, the longer len array to B.
        Binary search A.
            Calc m = (l+r)//2
            Calc n = half_len - m - 2
            Aleft = A[m]
            ...
            Bleft = A[n]
            Bright = A[n+1]
            (Need to handle out of range indexing later...)
            if (Aleft <= Bright) and (Bleft <= Aright):
                We found the median.
                If total_len is odd, then return min(Aright, Bright)
                If total_len is even, then return (max(Aleft, Bleft) + min(Aright, Bright)) / 2
            If Aleft > Bright:
                A is too big, so shrink it with r = m - 1
            Otherwise,
                l = m + 1
        Time: O(log(min(m, n)))
        Space: O(1)
        Edge: m and n could be 0, so need to handle m going negative during search.
        '''
