...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        Set the smaller len arr to A, and the longer len arr to B.
        l, r = 0, len(A)-1.
        Loop while True:
            m = (l + r) // 2
            n = half_len - m - 2
            ...
            if Aleft <= Bright and Bleft <= Aright:
                We found the median. Determine if the array is even or odd. If odd, return min(Aright, Bright). If
                even, return (max(Aleft, Aright) + min(Bleft, Bright)) / 2.
            Else if Aleft > Bright:
                A is too big, so search left with r = m - 1
            Otherwise
                A is too small, so search right with l = m + 1
        Time: O(log(min(m, n)))
        Space: O(1)
        Edge: Could be 0 vals in either m or n.
        '''
