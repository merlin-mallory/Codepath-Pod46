...
class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        Loop while l < r.
            Calc two_sum.
            If two_sum > target, we need to reduce it by decrementing the r pointer.
            If two_sum < target, we need to increase it by incrementing the l pointer.
            Otherwise, we've got a match, so return [l+1, r+1]
        Time: O(n)
        Space: O(1)
        Edge: None, guarenteed solution
        """
        l, r = 0, len(numbers)-1
        while l < r:
