class Solution:
    def twoSum(self, numbers: List[int], target: int) -> List[int]:
        ...
        1. Initialize left and right pointer to 0 and 1. (We are assuming length >= 2)
        2. Loop until numbers[left] + numbers[right] = target.
            3. Calculate current_sum.
            4. If current_sum == target, return [left+1, right+1]
            4. If current_sum < target, move right +1
            5. If current_sum > target, move left +1
        """
        l, r = 0, len(numbers) - 1
        while l < r:
