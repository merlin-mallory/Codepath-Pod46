...
class Solution:
    def swimInWater(self, grid: List[List[int]]) -> int:
        ...
        visited_set = set()
        while minheap:
            cur_val, old_r, old_c = heapq.heappop(minheap)
            if ((old_r,old_c)) in visited_set: continue
            visited_set.add((old_r, old_c))
            if (old_r == (rows-1)) and (old_c == (cols-1)): return cur_val
            for r_mod, c_mod in dirs:
                r = old_r + r_mod
                c = old_c + c_mod
                if (rows > r >= 0) and (cols > c >= 0) and ((r,c) not in visited_set):
                    max_val = max(cur_val, grid[r][c])
                    heapq.heappush(minheap, (max_val, r, c))
