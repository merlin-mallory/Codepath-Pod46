class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        set_of_followees.add(userId)
        for followee in set_of_followees:
            if followee in self.user_to_tweets:
                i = len(self.user_to_tweets[followee]) - 1
                count, tweetId = self.user_to_tweets[followee][i]
                heapq.heappush(maxheap, [count, followee, tweetId, i])
        final_arr = []
        while maxheap and (len(final_arr) < 10):
            count, followee, tweetId, i = heapq.heappop(maxheap)
            final_arr.append(tweetId)
            if i > 0:
                count, tweetId = self.user_to_tweets[followee][i-1]
                heapq.heappush(maxheap, [count, followee, tweetId, i-1])
        return final_arr
