class Twitter:
    ...
    def getNewsFeed(self, userId: int) -> List[int]:
        ...
        for followeeId in self.user_to_followees[userId]:
            if followeeId in self.user_to_tweets:
                i = len(self.user_to_tweets[followeeId])-1
                count, tweetId = self.user_to_tweets[followeeId][i]
                heapq.heappush(maxheap, [count, tweetId, followeeId, i])
        final_arr = []
        while maxheap and (len(final_arr) < 10):
            count, tweetId, followeeId, i = heapq.heappop(maxheap)
            final_arr.append(tweetId)
            if i > 0:
                next_i = i - 1
                next_count, next_tweetId = self.user_to_tweets[followeeId][next_i]
                heapq.heappush(maxheap, [next_count, next_tweetId, followeeId, next_i])
        return final_arr
    ...
    def unfollow(self, followerId: int, followeeId: int) -> None:
        ...
        '''
        if followerId in self.user_to_followees:
