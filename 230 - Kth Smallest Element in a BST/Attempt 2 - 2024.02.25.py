...
class Solution:
    def kthSmallest(self, root: Optional[TreeNode], k: int) -> int:
        ...
        Plan:
        Recursive Tree Traversal with in-order DFS
        Create dfs(node, count)
            Recurse left with (node.left, k)
            Count += 1
            Check if count == k, and if so, update kth_node[0], and return.
            Recurse right with (node.right, k)
        Return kth_node[0]
        '''
        stack = []
        while True:
