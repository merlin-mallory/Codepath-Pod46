...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        Plan:
        Binary Search
        Set min_k to the max of piles.
        Binary search from 1 to max(min_k).
            Calc cur_k, cur, and set time_to_finish to 0.
            Loop through piles, and accumulate time_to_finish using ceiling function of piles[i]/cur_k.
            if time_to_finish <= h, then we successfully ate the piles, so update min_k, and r = m-1 to search for a
            potentially better solution. Otherwise, l = m+1
        Return min_k
        Time: O(n)
        Space: O(1)
        Edge: None
        ...
        while l <= r:
            ...
            time_to_finish = 0
            for pile in piles:
                time_to_finish += math.ceil(pile / cur_k)
            if time_to_finish <= h:
