...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        Loop while l <= r.
            cur_k = (l + r) // 2
            time_to_finish = 0
            Loop through piles, and accumulate ceiling((pile/cur_k)) to time_to_finish.
            Check if time_to_finish <= h. If so, update min_k, and search left for a better result with r = cur_k -
            1. Otherwise, search right for a valid result with l = cur_k + 1
        Return min_k.
        Time: O(n log n)
        Space: O(1)
        Edge: None
