...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        Binary Search
        min_k = max(piles)
        Binary search between 1 and min_k.
            Calc cur_k = (l + r) // 2
            time_to_finish = 0
            Loop through piles, update time_to_finish
            If time_to_finish < h, then update min_k, and search left for a better solution (r = cur_k - 1).
            Otherwise search right for a valid solution (l = cur_k + 1)
        Return min_k
        Time: O(n log n)
        Space: O(1)
        Edge: None
