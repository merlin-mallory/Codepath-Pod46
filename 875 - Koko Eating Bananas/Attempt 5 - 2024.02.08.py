...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        Plan:
        Binary Search
        Find the max value in piles. Our minimum k will be somewhere between 1 and the max value of piles,
        and we will do binary search to find that value of k.
        l = 1, r = max(piles), min_k = max(piles).
        while l <= r:
            m = (l+r) // 2
            time_to_finish = 0
            Loop through piles using m for speed. Add +1 per time increment
            After the loop finishes, compare time_to_finish with h. If time_to_finish <= h, then we found a valid
            result, so update min_k, and then search left for a better min value with r = m - 1. Otherwise search
            right with l = m + 1.
        Return min_k
        Time: O(n * log(n))
        Space: O(1)
