...
class Solution:
    def minEatingSpeed(self, piles: List[int], h: int) -> int:
        ...
        Plan:
        Binary Search between 1 and the max of piles. At each iteration, calc the mid, and check if koko can finish
        all of the bananas (r = cur_k - 1) or not (l = cur_k + 1). Update min_k each iteration.
        Time: O(log(max(piles[i])))
        Space: O(1)
        Edge: None
        '''
        import math
