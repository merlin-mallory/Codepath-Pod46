...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        3. Loop while l <= r.
            m = (l+r)//2.
            m_val = nums[m]
            If nums[l] < nums[r], then we have a sorted subarray, so update min_val and break.
            If m_val > nums[r], then we are in the higher half of the array, so l = m + 1.
            Otherwise, we are in the lower half of the array, so r = m - 1.
        4. Return min_val
        """
        min_val = nums[0]
        l, r = 0, len(nums)-1
