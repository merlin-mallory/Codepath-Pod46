...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        while l <= r:
            if nums[l] < nums[r]:
                # We found a sorted subarray, so update the min_val with the leftmost value, and return.
            ...
            if cur_val >= nums[r]:
                # We are in the left subarray, so the min will be to the right.
                l = m + 1
            else:
                # We are in the right subarray, so the min will be to the left.
                r = r - 1
        return min_val
