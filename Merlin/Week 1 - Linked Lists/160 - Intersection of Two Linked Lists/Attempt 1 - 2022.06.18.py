...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        2. While the two pointers are not pointing at the same node,
            3. If A is None, then A = headB, else A = A.next
            4. If B is None, then B = headA, else B = B.next
        5. Return A (it will either point to the intersection node, or None if there is no node)
        6. Time: O(m+n), where m = len(head1), and n = len(head2).
        7. Space: O(1)
        '''
