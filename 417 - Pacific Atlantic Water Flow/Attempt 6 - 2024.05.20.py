...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def explore(r,c,cur_set,cur_max):
            if (r < 0) or (c < 0) or (r == rows) or (c == cols) or ((r,c) in cur_set) or (heights[r][c] < cur_max):
                return
            cur_set.add((r,c))
            cur_max = max(cur_max, heights[r][c])
            explore(r+1,c,cur_set,cur_max)
            explore(r-1,c,cur_set,cur_max)
            explore(r,c+1,cur_set,cur_max)
            explore(r,c-1,cur_set,cur_max)

        for r in range(rows):
            explore(r, 0, pacific_set,0)       # left border
            explore(r, cols-1, atlantic_set,0)    # right border
        for c in range(cols):
            explore(0, c, pacific_set,0)       # top border
            explore(rows-1, c, atlantic_set,0)    # bottom border
