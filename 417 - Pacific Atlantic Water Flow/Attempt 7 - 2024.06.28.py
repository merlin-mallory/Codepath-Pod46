...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        def explore(r, c, ocean_set, cur_min):
            if (r < 0) or (c < 0) or (r == rows) or (c == cols) or ((r,c) in ocean_set) or heights[r][c] < cur_min:
                return
            ocean_set.add((r, c))
            cur = heights[r][c]
            for r_mod, c_mod in dirs:
                new_r, new_c = r+r_mod, c+c_mod
                explore(new_r, new_c, ocean_set, cur)

        for r in range(rows):
            explore(r, 0, pacific_set, 0)       # Left side
            explore(r, cols-1, atlantic_set, 0)    # Right side
        for c in range(cols):
            explore(0, c, pacific_set, 0)       # Top side
            explore(rows-1, c, atlantic_set, 0)    # Bottom side
