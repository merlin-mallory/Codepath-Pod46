...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        atlantic_set = set()
        final_arr = []
        def backtrack(r, c, cur_set, prev_height):
            if (r < 0) or (c < 0) or (r == rows) or (c == cols) or ((r,c) in cur_set) or (heights[r][c] < prev_height):
                return
            cur_set.add((r, c))
            backtrack(r+1, c, cur_set, heights[r][c])
            backtrack(r-1, c, cur_set, heights[r][c])
            backtrack(r, c+1, cur_set, heights[r][c])
            backtrack(r, c-1, cur_set, heights[r][c])

        for r in range(rows):
            backtrack(r, 0, pacific_set,0)   # Explore left edge
            backtrack(r, cols-1, atlantic_set,0)  # Explore right edge
        for c in range(cols):
            backtrack(0, c, pacific_set,0)   # Explore top edge
            backtrack(rows-1, c, atlantic_set,0) # Explore bottom edge
        for r,c in pacific_set:
            if (r,c) in atlantic_set: final_arr.append([r,c])
        return final_arr
