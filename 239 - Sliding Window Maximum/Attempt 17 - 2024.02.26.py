...
class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        Plan:
        Sliding window with monotonic decreasing order deque. Deque will store indexes referencing values.
        Create deque, final_arr.
        l, r = 0,  0
        Loop while r < len(nums).
            Clean the left side of the deque. If deque[-1] < l, popleft().
            Clean the right side of the deque. If nums[deque[-1]] >= cur, popright()
            Append rto the deque
            If r+1 >= k, append nums[deque[0]] to final_arr, l++.
            r++
        Return final_arr
        Time: O(n)
        Space: O(n)
