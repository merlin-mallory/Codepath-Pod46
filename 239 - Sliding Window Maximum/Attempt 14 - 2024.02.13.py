...
class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        Plan:
        Sliding Window
        Create a monotonic decreasing order deque.
        l, r = 0, 0
        Loop while r < len(nums).
            Clean the left side of the deque. If deque[0] < l, then dequeleft.pop().
            Clean the right side of the deque. If nums[deque[-1]], then deque.pop().
            Append r to the deque.
            If r+1 >= k, append deque[0] to final_arr, and l++
        Return final_arr.
        Time: O(n)
        Space: O(n)
        Edge: None
