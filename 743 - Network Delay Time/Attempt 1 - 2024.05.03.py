...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        minheap = [(0,k)]   # Time, Point
        visited_set = set()
        total_time = 0
        while minheap and (len(visited_set) != n):
            t1, d1 = heapq.heappop(minheap)
            if d1 in visited_set: continue
            total_time = t1
            visited_set.add(d1)
            for t2, d2 in source_to_dest[d1]:
                if d2 not in visited_set:
                    heapq.heappush(minheap, (t1+t2, d2))
