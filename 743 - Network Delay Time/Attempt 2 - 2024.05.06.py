...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        minheap = [(0,k)]   # [dist, point]
        final_time = 0
        visited_set = set()
        while minheap and (len(visited_set) != n):
            cur_time, cur_dest = heapq.heappop(minheap)
            if cur_dest in visited_set: continue
            final_time = cur_time
            visited_set.add(cur_dest)
            for nei_t, nei_dest in source_to_dest[cur_dest]:
                if nei_dest not in visited_set:
                    heapq.heappush(minheap, (cur_time + nei_t, nei_dest))
        return final_time if (len(visited_set) == n) else -1
