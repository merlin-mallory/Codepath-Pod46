...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        Time: O(k*n)
        Space: O(n)
        Edge: None, but need to handle possibility of no such route
        '''
        prices = [float('inf')] * n
        prices[src] = 0
        for i in range(k+1):
            temp_prices = prices[:]
            for source, dest, edge in flights:
                if (prices[source] + edge) < temp_prices[dest]:
                    temp_prices[dest] = prices[source] + edge
            prices = temp_prices
