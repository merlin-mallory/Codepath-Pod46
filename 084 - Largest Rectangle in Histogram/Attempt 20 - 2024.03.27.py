...
class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        Plan:
        Stack
        Loop through heights, if we encounter a decreasing size height, pop from stack,
        calc cur_area, update max_area, update start. Then append [start, cur] to the stack.
        Loop through stack, the remaining values stretch beyond the right bound. So calc those rectangle's cur_area,
        and update max_area.
        Return max_area
        Time: O(n)
        Space: O(n)
