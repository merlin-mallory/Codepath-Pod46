...
class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        Plan:
        Stack
        Init stack = []. This will hold [value, index] pairs. Also init max_area = 0.
        l = 0, r = 0
        Loop while r < len(heights).
            cur_val = heights[r]
            while cur_val < stack[-1][0]:
                stack_val, stack_ind = stack.pop()
                cur_area = min(stack_val, cur_val) * (r - stackInd)
                Update max_area.
        ...
        while r < len(heights):
            ...
            while stack and cur_val < stack[-1][0]:
                stack_val, stack_ind = stack.pop()
                cur_area = stack_val * (r - stack_ind)
        ...
        while stack:
            stack_val, stack_ind = stack.pop()
            cur_area = stack_val * (len(heights) - stack_ind)
            max_area = max(max_area, cur_area)
