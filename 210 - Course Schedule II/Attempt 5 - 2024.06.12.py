...
class Solution:
    def findOrder(self, numCourses: int, prerequisites: List[List[int]]) -> List[int]:
        ...
        Plan:
        Graph Traversal with DFS backtracking
        Time: O(n) ?
        Space: O(n)
        Edge: prerequisites coulde be empty, numCourses can be 0
        '''
        import collections
        ...
        cycle_set = set()
        def explore(i):
            if i in cycle_set: return False
            if i in visited_set: return True
            cycle_set.add(i)
            for prereq in course_to_prereq[i]:
