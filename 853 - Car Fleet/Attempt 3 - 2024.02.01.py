...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # Create cars array which will contain pairs of [position, time_to_finish], sorted in reverse order by position.

        for i in range(len(position)):
            cars.append([position[i], (target - position[i]) / speed[i]]) # Position and time_to_finish

        cars = sorted(cars)[::-1]

        # Loop through cars, calculate time_to_finish, add to stack. compare stack[-1] with stack[-2].
        for i in range(len(cars)):
            stack.append(cars[i][1])
            if len(stack) >= 2 and stack[-1] <= stack[-2]:
                stack.pop()

        # The length of the stack will contain the number of fleets
        return len(stack)
