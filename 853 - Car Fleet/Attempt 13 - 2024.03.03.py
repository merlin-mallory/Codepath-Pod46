...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        Create cars. Array of [position[i], (target - position[i])/speed[i])] pairs.
        Reverse sort cars.
        Loop through cars.
            Add car to the stack.
            Compare stack[-1][1] with stack[-2][1]. If <=, then that means the -1 car will form a fleet with the -2
            car. So pop the -1 car from the stack, and keep the -2 car on the stack to check for future fleets.
        Return len(stack)
        Time: O(n log n)
        Space: O(n)
        Edge: None
