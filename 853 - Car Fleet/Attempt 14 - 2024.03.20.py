...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        Create cars array. Contains [position[i], (target - position[i])/speed[i]] pairs.
        Reverse sort cars.
        Create stack.
        Loop through cars.
            Append car[i] to the stack.
            Compare stack[-1][1] with stack[-2][1]. If <, then the car at the top of the stack will form a fleet with
            the -2 car. So pop the stack.
        Return the len of the stack.
        Time: O(n)
        Space: O(1) or O(n) depending upon classification of return array.
        Edge: If two cars reach the target at the same time, they form a single fleet.
        '''
        cars = []
        stack = []
        for i, val in enumerate(position):
            cars.append([val, (target - val)/speed[i]])
        cars = sorted(cars)[::-1]
        for car in cars:
            stack.append(car)
