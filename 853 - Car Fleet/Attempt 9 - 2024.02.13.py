...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        Create cars. [position[i], time_to_finish].
        Reverse sort cars.
        Loop through cars, processing the car closest to the finish line first.
            Append car[i] to the stack.
            Compare stack[-1][1] with stack [-2][1]. If <, then the -2 car will form a fleet with the -1 car,
            so we can pop stack[-1]
        Return the len of the array (will contain number of fleets)
        Time: O(n log n)
        Space: O(n)
        Edge: None
        '''
        ...
        for i, pos in enumerate(position):
            cars.append([pos, ((target - pos)/speed[i])])
        cars = sorted(cars)[::-1]
        i = 0
        while i < len(cars):
            stack.append(cars[i])
