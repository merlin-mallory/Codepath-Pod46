...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        Create cars, which is an array that holds [position[i], time_to_finish] pairs.
        Reverse sort cars.
        Loop through cars.
            Append cars[i] to the stack.
            Compare stack[-1] to stack[-2]. If the time_to_finish of stack[-1] is greater, then that means it will
            form a fleet with the slower car at stack[-2]. So just pop stack[-1].
        Return the len of the stack.
        Time: O(n*log(n))
        Space: O(n)
        Edge: None, but make sure that if two cars reach end at same time, they are considered as one car fleet.
        '''
        stack = []
        cars = []
