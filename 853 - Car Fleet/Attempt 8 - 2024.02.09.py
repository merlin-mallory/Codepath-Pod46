...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        1. Create cars array. Contains [position[i], time_to_finish] for each index in cars and speed.
        2. Reverse sort the cars array, so we can process the car closest to the finish first, followed by the rest
        in sequential order.
        3. Loop through the cars array. Append the cur_car to the stack. Then compare, is stack[-1][1] < stack[-2][
        1]? If so, that means the most recently appended car to the stack will form a fleet with the stack[-2] car.
        So pop the top of the stack to remove it from consideration.
        4. After the loop finishes, the length of the array will provide the number of fleets, so return it.
        Time: O(n)
        Space: O(n)
        Edge: None
        '''
