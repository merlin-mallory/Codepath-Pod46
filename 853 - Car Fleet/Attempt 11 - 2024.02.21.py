...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # Create cars array, which will contain [position[i], time_to_finish] pairs.
        stack = []
        cars = []
        for i in range(len(position)):
            cars.append([position[i], (target - position[i])/speed[i]])

        # Reverse sort the cars array
        cars = sorted(cars)[::-1]

        # Loop through the cars array, adding each car to the stack. Then compare stack[-1][1] and stack[-2][1]. If
