...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        Create cars array. Loop through positions, and append [position[i], (target - position[i])/speed[i]] to cars.
        So the zero index will contain the starting position of the car, and the one index will contain the time it
        takes for the car to reach the finish line.
        Reverse sort the cars array, so they'll be in timeline order.
        Loop through cars and add them to the stack.
        Also check if there's a car on the stack, and then compare the current
        car's speed with the stack car's speed. If the stack car's speed is lower than the current car's speed,
        then that means those cars would form a fleet on top of the stack car's location. So pop the stack.
        At the end of this algorithm, the stack will contain a count of the number of fleets, so return len(stack).
        '''
        cars = []
        stack = []
