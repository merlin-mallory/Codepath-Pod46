...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Stack
        Create cars array [position[i], time_to_finish].
        Reverse sort cars array, so we'll process the car closest to the finish line first.
        Loop through cars.
            Append car to the stack.
            Compare stack[-1][1] with stack[-2][1]. If the -1's val is less than the -2, then that means the -1 car
            will form a fleet with the -2 car. So pop the -1 car from the stack.
        Return len(cars)
        Time: O(n)
        Space: O(n) or O(1) depending upon if the stack counts
        '''
        cars = []
        stack = []
