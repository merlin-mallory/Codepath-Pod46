class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        Plan:
        Brute Force
        1. Loop until finished = len(position).
            2. Loop through position.
                3. If value = null, then skip
                4. Add position + speed.
                5. If value >= target, then fleet++, then scan to find number of cars for finished++, and then value
                = null
        6. Return finished
        Time: O(n^2)
        '''
        ...
        cars = sorted(cars)[::-1]   # Reverse Sorted Order
