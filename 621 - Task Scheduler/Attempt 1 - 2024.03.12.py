class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        Plan:
        Create a task_dict (counter dict)
        Create a maxheap and a cd_deque.
        Negify task_dict.values()
        Heapify task_dict.values()
        time = 0
        Loop while maxheap or cd_deque.
            time += 1
            if maxheap:
                cur_count, cur_time = heappop
                if cur_count != 1, cd_deque.append([cur_count + 1, cur_time + n])
            if cd_deque:
                if cd_deque[0][1] <= cur_time:
                    heapq.heappush(cd_deque.popleft()
        return time
        Time: O(n)
        Space: O(n)
        Edge: n could be 0
        ...
        while maxheap or cd_deque:
            ...
            if maxheap:
                ...
                if cur_count != -1: cd_deque.append([cur_count + 1, time + n])
            if cd_deque:
                if cd_deque[0][1] <= time:
