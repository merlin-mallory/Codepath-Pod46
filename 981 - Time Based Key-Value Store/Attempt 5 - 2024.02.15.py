class TimeMap:
    ...
    def __init__(self):
        ...
        Plan:
        Create dict.
        Set: Check if dict[key] in dict. If so, delete the key. Append [value, timestamp] to dict[key].
        Get: Grab dict_arr = dict[key]. Set default_str = "". Do binary search on dict_arr. If dict_arr[m][1] ==
        input_timestamp, then return dict_arr[m][0]. Else if dict_arr[m][1] < input_timestamp, update default_str
        with dict_arr[m][0], and r = m - 1. Otherwise l = m +1. Return default_str.
        Time: O(n) for set, O(log(n)) for get.
        Space: O(n) for set, O(1) for get.
        Edge: None
        '''
        import collections
    ...
    def get(self, key: str, timestamp: int) -> str:
        default_str = ""
        dict_arr = self.dict[key]
        l, r = 0, len(dict_arr)-1
        while l <= r:
            m = (l + r) // 2
            if dict_arr[m][1] == timestamp:
                return dict_arr[m][0]
            elif dict_arr[m][1] < timestamp:
                default_str = dict_arr[m][0]
                l = m + 1
            else:
                r = m - 1
        return default_str
