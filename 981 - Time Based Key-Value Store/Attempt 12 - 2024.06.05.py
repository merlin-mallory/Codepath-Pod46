class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        list_of_timestamps = self.dict[key]
        l, r = 0, len(list_of_timestamps) - 1
        default_str = ""
        while l <= r:
            m = (l + r) // 2
            cur_timestamp, cur_val = list_of_timestamps[m]
            if cur_timestamp == timestamp: return cur_val
            elif cur_timestamp > timestamp: r = m - 1
            else:
                default_str = cur_val
                l = m + 1
        return default_str
