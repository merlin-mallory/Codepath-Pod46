class TimeMap:
    ...
    Init: Create dict. Keys: Strings, Values: List of [value, timestamp] pairs.
    Set: Append [value, timestamp] to dict[key].
    Get: Grab dict[key], assign to cur_arr. Set default_str to "". Binary search through cur_arr[i][1] for a match with
    target, if found, return cur_arr[i][0]. Otherwise if cur_arr[i][1] < target, update default_str. At the end of
    the search, return default_str.
    Time: O(1) for init, set. O(log(n)) for get.
    Space: O(1) for init, set, get
    Edge: None
    '''
    ...
    def get(self, key: str, timestamp: int) -> str:
        ...
        while l <= r:
            m = (l + r) // 2
            cur_val, cur_timestamp = cur_arr[m]
