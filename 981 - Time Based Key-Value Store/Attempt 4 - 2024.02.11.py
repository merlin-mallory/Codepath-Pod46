class TimeMap:
    ...
    Plan:
    Binary Search
    Set: Pretty simple add to hashmap.
    Get: Fetch the input's value, which will be an array that contains [timestamp, strVal] pairs sorted according to
    timestamp. Set default_str = "". Binary search through this subarray. If we find a perfect match, return the
    item's strVal. Otherwise, if dict_timestamp < input_timestamp, update default_str. At the end of the binary
    search, if we haven't found a perfect match, then return default_str.
    Time: O(n)
    Space: O(n)
    Edge: None
    '''
    ...
    def get(self, key: str, timestamp: int) -> str:
        subarray = self.hashmap[key]
        l, r = 0, len(subarray)-1
        default_str = ""
        while l <= r:
            m = (l + r) // 2
            if subarray[m][0] == timestamp:
                return subarray[m][1]
            elif subarray[m][0] < timestamp:
                default_str = subarray[m][1]
                l = m + 1
            else:
                r = m - 1
        return default_str
