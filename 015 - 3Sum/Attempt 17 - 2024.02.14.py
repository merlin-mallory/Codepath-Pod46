...
class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        ...
        Two Pointers
        Sort nums.
        Loop through nums, handle duplicates, grab left_val.
            l = i+1
            r = len(nums)-1
            Solve three_sum. If < 0, increase with l++. If > 0, decrease with r--. If ==, append [left_val, nums[l],
            nums[r]], l++, and handle duplicates.
            i++
        Return final_arr.
        Time: O(n^2)
        Space: O(1)
        Edge: Handle out-of-bounds with duplicates
        """
        nums.sort()
        final_arr = []
        i = 0
