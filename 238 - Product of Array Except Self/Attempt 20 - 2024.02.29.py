...
class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        Plan:
        Init answer array to [1] * len(nums)
        Loop through nums once, updating prefix and then nums[i-1].
        Loop backwards through nums once, updating postfix and the nums[i+1].
        Return answer.
        Time: O(n)
        Space: O(1)
        '''
