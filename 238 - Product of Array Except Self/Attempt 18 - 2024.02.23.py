...
class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        Plan:
        Init answer = [1] * len(nums)
        Init prefix = 1
        Loop through nums, update prefix, update answer's index with corresponding value from nums.
        Init postfix = 1
        Loop backwards through nums, update postfix, update answer's index with corresponding value from nums.
        Time: O(n)
        Space: O(1)
        Edge: None
        '''
