class MedianFinder:
    ...
    Plan:
    Heap
    Create left_max_heap and right_min_heap. Whenever we add, check to see if the val should go into either the left
    or right seg. And then total_len++, and then rebalance the left and right segments to be within 1 length of each
    other. For findMedian, determine if the total_len is odd or even, and return the appropriate value from the heaps.
    Time: O(n log n)
    Space: O(n)
    Edge: None
    '''
