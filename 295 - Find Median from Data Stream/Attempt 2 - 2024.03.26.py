class MedianFinder:
    ...
    Plan:
    Init: Create two heaps. left_max_heap and right_min_heap. Also create a total_len counter.
    addNum: Analyze the two heaps to determine which one the num should go into. Add the val to the proper heap. Then
    rebalance the heaps so the lengths stay within 1 length of each other. Also increment total_len.
    findMedian: Determine if the total_len is odd (return the val from the larger size heap) or even (return the
    average of the top of both heaps).
    '''
