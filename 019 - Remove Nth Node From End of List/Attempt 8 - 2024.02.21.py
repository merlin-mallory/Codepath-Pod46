...
class Solution:
    def removeNthFromEnd(self, head: Optional[ListNode], n: int) -> Optional[ListNode]:
        ...
        '''
        # Plan: Linked List traversal with slow/fast pointers.
        # Set up dummy
        dummy = ListNode(-1, head)
        slow = dummy
        fast = head
        # Loop the fast pointer n times
        while fast and (n>0):
            fast = fast.next
            n -= 1

        # Loop slow and fast pointers together until fast reaches None.
        while fast:
            slow = slow.next
            fast = fast.next

        # Slow should stop before the targeted node for deletion, so delete slow.next
