...
class Solution:
    def removeNthFromEnd(self, head: Optional[ListNode], n: int) -> Optional[ListNode]:
        ...
        Create dummy
        Increment fast n times to create an n size gap between head and fast.
        Set slow = head
        Loop until fast == None
            slow = slow.next
            fast = fast.next
        Eventually, when fast reaches None, slow.next will be the node targeted for deletion. So set slow.next to
        slow.next.next
        Return dummy.next
        Time: O(n)
        Space: O(1)
