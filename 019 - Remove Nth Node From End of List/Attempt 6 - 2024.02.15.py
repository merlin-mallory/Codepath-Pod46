...
class Solution:
    def removeNthFromEnd(self, head: Optional[ListNode], n: int) -> Optional[ListNode]:
        ...
        Plan:
        Create dummy
        Create slow (linked to dummy) and fast pointers.
        Iterate the fast pointer through the head n times.
        Loop while there's a fast pointer.
            Advance both slow and fast pointer.
        Eventually, the slow pointer will land on the node before the desired deletion point.
        Delete the desired node.
        Return dummy.next.

        Time: O(n)
        Space: O(1)
        Edge: Could only be 1 node.
        '''
