...
class Solution:
    def removeNthFromEnd(self, head: Optional[ListNode], n: int) -> Optional[ListNode]:
        ...
        Plan:
        Use a fast pointer and loop through head n times.
        Set slow pointer to head.
        Loop slow and fast together until fast = NOne
        Slow should stop right before the desired node for deletion, reassign slow.next to slow.next.next
        Return head
        Time: O(n)
        Space: O(1)
        Edge: None
