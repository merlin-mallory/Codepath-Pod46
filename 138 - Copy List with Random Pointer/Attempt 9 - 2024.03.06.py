...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        Create old_to_new dict. Keys: nodes in head, values: matching nodes in the copy.
        Loop through head.
            Create new TreeNodes in old_to_new using cur.val
        Loop through head
            Grab the cur node via old_to_new
            Assign the .next and .random pointers using the dict.
        ...
        cur = head
        while cur:
            old_to_new[cur] = TreeNode(cur.val)
            cur = cur.next
        cur = head
        while cur:
