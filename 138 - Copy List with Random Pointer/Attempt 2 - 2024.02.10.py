...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        Plan:
        Linked List that rewires copies via a dictionary.
        Time: O(n)
        Space: O(n)
        ...
        dummy = cur
        while cur:
            copied_node = old_to_copy[cur]
            copied_node.random = old_to_copy[cur.random]
            copied_node.next = old_to_copy[cur.next]
            cur = cur.next
        return old_to_copy[dummy]
