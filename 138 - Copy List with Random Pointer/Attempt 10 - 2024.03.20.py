...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        Loop through head.
            Create new LinkNode in old_to_new dict, using cur's value.
        Loop through head
            Grab the cur_node.
            Set cur_node.next and cur_node.random using the old_to_new dict.
        Return old_to_new[head]
        Time: O(n)
        Space: O(n) or O(1), depending upon the final result counting towards memory.
        Edge: n could be 0, need to handle Nones in old_to_new_dict
        '''
