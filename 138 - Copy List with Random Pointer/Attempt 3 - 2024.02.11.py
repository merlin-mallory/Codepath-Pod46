...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        Plan:
        Linked List
        Create old_to_copy dict. Keys: Nodes from head, Values: Matching copies in the new LL.
        Loop through head and construct the new LL with just values.
        Loop through head a second time, and use the dict to assign the .next and .random pointers
        Also make sure to handle the Null pointer at the end.
        Return the node in the dict that matches the head.
        Time: O(n)
        Space: O(n) (Or maybe O(1) depending upon interpretation)
        Edge: There could be an empty list.
        '''
