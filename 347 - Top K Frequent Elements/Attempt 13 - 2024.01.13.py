from typing import List
class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        '''
        https://leetcode.com/problems/top-k-frequent-elements/

        Given an integer array nums and an integer k, return the k most frequent elements. You may return the answer
        in any order.

        Input: nums = [1,1,1,2,2,3], k = 2
        Output: [1,2]

        Input: nums = [1], k = 1
        Output: [1]

        Constraints:

        1 <= nums.length <= 10^5
        k is in the range [1, the number of unique elements in the array].
        It is guaranteed that the answer is unique.

        Follow up: Your algorithm's time complexity must be better than O(n log n), where n is the array's size.
        '''
        import collections
        counts_dict = collections.defaultdict(int)

        for num in nums:
            counts_dict[num] += 1

        import heapq
        heap = []
        for (key, value) in counts_dict.items():
            current_tup = (-value, key)
            heapq.heappush(heap, current_tup)

        result = []
        for i in range(k):
            _, current_num = heapq.heappop(heap)
            result.append(current_num)

        return result



answer = Solution()
result = answer.topKFrequent([1,1,1,2,2,3], 2) # [1,2]
print(result)

result2 = answer.topKFrequent([1], 1)  # [1]
print(result2)

# Note: We need the negative value (frequency) because heapq does a minheap by default.
# So heapq.heapop() will pop off the smallest element.
# But in this problem, we're looking for max values. Adding the negative essentially makes it a maxheap.