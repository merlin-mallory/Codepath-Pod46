class Solution:
    def isValidSudoku(self, board) -> bool:
        ...
        for m in range(9):
            for n in range(9):
                current_square = board[m][n]
                if current_square == ".":
                    continue

                if current_square in rows_set[m]:
                    return False
                else:
                    rows_set[m].add(current_square)

                if current_square in columns_set[n]:
                    return False
                else:
                    columns_set[n].add(current_square)

                box_calc = (m // 3) * 3 + (n // 3)

                if current_square in box_set[box_calc]:
                    return False
                else:
                    box_set[box_calc].add(current_square)
